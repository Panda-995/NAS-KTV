...
# Locate original audio block start after MV ternary
start = text.find('      ) : (\n        /* ===== ')
if start == -1:
    raise SystemExit('audio block start not found')
# locate closing of audio block before advanced controls usage
adv = text.find('      {/* AdvancedControlsPanel', start)
if adv == -1:
...
'''
path.write_text(text[:start] + new_audio + text[adv:], encoding='utf-8')
